/**
 * Vendored from @pdfsmaller/pdf-encrypt-lite@1.0.2 (MIT). Original copyright
 * PDFSmaller.com (https://pdfsmaller.com). We vendor a patched copy so the
 * permission integer (`/P`) can be supplied by the caller — the published
 * package hardcodes it. Keep this in sync if upstream gains a permissions
 * parameter, at which point we can drop the vendored copy.
 *
 ...
 * @license MIT
 *
 * Implements the PDF Standard Security Handler (Algorithm 2 & 3 from the PDF spec).
 */
 ...
 */
export async function encryptPDF(pdfBytes, userPassword, ownerPassword = null, permissions = 0xFFFFFFFC) {
  try {
    // Load the PDF
    ...
    // Permissions (`/P`) are supplied by the caller; default 0xFFFFFFFC = all allowed.
    // This value is both hashed into the encryption key (Algorithm 2) and written
    // as `/P`, so it must be chosen here, before the key is derived.
